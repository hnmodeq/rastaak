...
`;
    ...
    if (
      ...
      const environment = {
        ...
        sky: {
          zenithColor: hexLit(asHexNumber(body.environment?.sky?.zenithColor, 0x02050e)),
          upperColor: hexLit(asHexNumber(body.environment?.sky?.upperColor, 0x081a31)),
          horizonColor: hexLit(asHexNumber(body.environment?.sky?.horizonColor, 0x27375c)),
          ...
          moonGlow: Math.min(3, Math.max(0, asFinite(body.environment?.sky?.moonGlow, 1))),
          horizonGlow: Math.min(3, Math.max(0, asFinite(body.environment?.sky?.horizonGlow, 1))),
          starDensity: Math.min(2, Math.max(0, asFinite(body.environment?.sky?.starDensity, 1))),
          starIntensity: Math.min(3, Math.max(0, asFinite(body.environment?.sky?.starIntensity, 1))),
          exposure: Math.min(3, Math.max(0, asFinite(body.environment?.sky?.exposure, 1))),
        },
        horizon: {
